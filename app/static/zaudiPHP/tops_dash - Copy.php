...
function get_today_entries(PDO $db, string $day): array {
    try {
        $stmt = $db->prepare("SELECT log_id, logged_at, category, venue, item, portion_pct, est_calories, notes FROM food_log WHERE DATE(logged_at) = :day ORDER BY logged_at ASC");
...
// Selected day nav
$selected = (isset($_GET['d']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['d'])) ? $_GET['d'] : date('Y-m-d');
$is_today  = $selected === date('Y-m-d');
$prev_day  = date('Y-m-d', strtotime($selected . ' -1 day'));
$next_day  = date('Y-m-d', strtotime($selected . ' +1 day'));
$show_next = $selected < date('Y-m-d');

try {
    $db        = get_db();
    $entries   = get_today_entries($db, $selected);
...
<body>
<header>
    <h1>TOPS &mdash; <?= $is_today ? 'Today' : date('M j', strtotime($selected)) ?></h1>
    <div class="links">
        <a href="?d=<?= $prev_day ?>">&larr;</a>
        <?php if ($show_next): ?><a href="?d=<?= $next_day ?>">&rarr;</a><?php endif ?>
        <?php if (!$is_today): ?><a href="?">Today</a><?php endif ?>
...
<div class="wrap">
    ...
    <div class="section">
        <div class="section-label"><?= $is_today ? 'Today' : date('l, M j', strtotime($selected)) ?></div>
    ...
    <div class="section">
        ...
        <div class="week-wrap">
            ...
            $max_cal = max(array_merge([$daily_target], array_column($week, 'total_cal')));
            $days_shown = [];
            for ($i = 6; $i >= 0; $i--) {
                $d   = date('Y-m-d', strtotime("-{$i} days"));
                $lbl = strtoupper(substr(date('D', strtotime($d)), 0, 1));
                $cal = isset($week_by_day[$d]) ? (int)$week_by_day[$d]['total_cal'] : 0;
                $days_shown[] = ['d' => $d, 'lbl' => $lbl, 'cal' => $cal, 'today' => $i === 0, 'selected' => $d === $selected];
            }
            $max_h = 55;
            ?>
            <div class="week-bars">
                <?php foreach ($days_shown as $ds):
                    $consumed  = min($ds['cal'], $daily_target);
                    $remaining = max(0, $daily_target - $ds['cal']);
                    $overage   = max(0, $ds['cal'] - $daily_target);
                    $scale     = $max_cal > 0 ? $max_h / $max_cal : 0;
                    $h_consumed  = max($consumed  > 0 ? 3 : 0, round($consumed  * $scale));
                    $h_remaining = max($remaining > 0 ? 2 : 0, round($remaining * $scale));
                ...
                <div class="bar-col" onclick="window.location='?d=<?= $ds['d'] ?>'" style="cursor:pointer">
                    ...
                    <div style="display:flex;flex-direction:column;align-items:center;width:100%;position:relative">
                        ...
                        <?php endif ?>
                        <?php if ($h_remaining > 0 && $ds['cal'] > 0): ?>
                        <div class="bar" style="height:<?= $h_remaining ?>px;background:#22c55e;opacity:0.35;border-radius:<?= $h_overage == 0 ? '3px 3px 0 0' : '0' ?>"></div>
                        <?php endif ?>
                        <?php if ($h_consumed > 0): ?>
                        <div class="bar" style="height:<?= $h_consumed ?>px;background:var(--accent);border-radius:0"></div>
                        <?php endif ?>
                    </div>
                    <span class="bar-lbl" style="<?= $ds['selected'] ? 'color:var(--text)' : '' ?>"><?= $ds['lbl'] ?></span>
                </div>
                <?php endforeach ?>
            </div>
        </div>
