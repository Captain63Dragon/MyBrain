...
### BotFunction Node Conventions
> **Module path:** Dynamic bots use `app.bots.db.mia` (Flask importlib, runs from project root).
> Vera uses `bots.db.vera` only because she has legacy hardcoded routes - she never hits `/bots/execute`.
> **Rule: any new bot using `/bots/execute` needs `app.` prefix.**
> **bot-id format:** Use dots only - `mia.tops.log.get`, not `mia.tops_log.get`. Underscores break bridge routing.
> **Verify by testing get:** load tools then call a get function - import errors surface immediately.
